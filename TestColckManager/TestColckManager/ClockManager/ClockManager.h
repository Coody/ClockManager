...
-(void)clearClock;
/**
 * @brief - 暫時停止（當有開始計時的時候才有用）
 */
-(void)pauseClock;
/**
 * @brief - 繼續計時（當時鐘是停止的時候才有用）
 */
-(void)startClock;
@end
...
/**
 * @brief - 取得一個倒數計時的鬧鐘（ Process Block ）， ClockManager 不做管理。
 * @param tempSecond 秒數
...
/**
 * @brief - 取得一個倒數計時的鬧鐘（ Start Block , Process Block , End Block ）， ClockManager 不做管理。。
 * @param tempSecond 秒數
 * @param startResponseBlock   : Start block
 * @param processResponseBlock : Process Block
 * @param endResponseBlock     : End Block
 */
-(NormiClock *)getRandomClockWithSecond:(NSUInteger)tempSecond 
                         withStartBlock:(void(^)(void))startResponseBlock
                       withProcessBlock:(void(^)(NSUInteger second))processResponseBlock
